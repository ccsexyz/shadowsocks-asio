...
ServerSession::ServerSession(
    asio::io_service &io_service, asio::ip::tcp::socket &&socket, config &config)
    : config_(config)
    , service_(io_service)
...
void ServerSession::destroyLater() { plusOneSecond(service_, std::move(socket_)); }

void ServerSession::async_read_some(Handler handler)
{
    socket_.async_read_some(
        asio::buffer(buf, 4096), [this, handler](std::error_code ec, std::size_t length) {
            if (!ec) {
...
void ServerSession::async_read(std::size_t len, Handler handler) { async_read(buf, len, handler); }

void ServerSession::async_read(char *buffer, std::size_t len, Handler handler)
{
    asio::async_read(socket_, asio::buffer(buffer, len),
        [this, handler, buffer](std::error_code ec, std::size_t length) {
            if (!ec) {
...
void ServerSession::async_write(char *buffer, std::size_t len, Handler handler)
{
    enc_->encrypt(buffer, len, buffer, len);
    asio::async_write(socket_, asio::buffer(buffer, len),
        [this, handler, buffer](std::error_code ec, std::size_t length) { handler(ec, length); });
}
...
{
    auto self = shared_from_this();
    async_read_with_timeout(enc_->getIvLen(), std::chrono::seconds(4),
...
void ServerSession::doGetRequest()
{
    ...
    async_read_with_timeout_1(
        1, std::chrono::seconds(1), [this, self](std::error_code ec, std::size_t length) {
            if (ec) {
                destroyLater();
                return;
            }
            ...
            default:
                approve = false;
                log_info("incorrect header from %s", address.c_str());
                break;
            }
...
{
    auto self = shared_from_this();
    async_read(lenIPv4 + lenPort, [this, self](std::error_code ec, std::size_t length) {
        if (ec) {
            return;
        }
        char *address = buf + 128;
        if (::inet_ntop(AF_INET, reinterpret_cast<void *>(buf), address, 1024) == nullptr) {
            return;
        }
        std::string name = address;
        std::string port = std::to_string(ntohs(*reinterpret_cast<uint16_t *>(buf + 4)));
...
{
    auto self = shared_from_this();
    async_read_with_timeout_1(lenIPv6 + lenPort, std::chrono::seconds(1),
        [this, self](std::error_code ec, std::size_t length) {
            if (ec) {
                return;
            }
            char *address = buf + 128;
            if (::inet_ntop(AF_INET6, reinterpret_cast<void *>(buf), address, 1024) == nullptr) {
                return;
            }
            std::string name = address;
            std::string port = std::to_string(ntohs(*reinterpret_cast<uint16_t *>(buf + 16)));
            doEstablish(name, port);
        });
}
...
void ServerSession::doGetDmRequest()
{
    ...
    async_read_with_timeout_1(
        1, std::chrono::seconds(1), [this, self](std::error_code ec, std::size_t length) {
            ...
            async_read_with_timeout_1((unsigned char)buf[0] + 2, std::chrono::seconds(1),
                [this, self](std::error_code ec, std::size_t length) {
                    if (ec || length < 2) {
                        return;
                    }
                    ...
                    std::string port
                        = std::to_string(ntohs(*reinterpret_cast<uint16_t *>(buf + length - 2)));
                    LOG_TRACE
                    doEstablish(name, port);
                });
        });
...
{
    log_info("connect to %s:%d", name.c_str(), port.c_str());
    LOG_TRACE
    auto self = shared_from_this();
    asio::ip::tcp::resolver::query query(name, port);
    resolver_.async_resolve(
        query, [this, self](std::error_code ec, asio::ip::tcp::resolver::iterator iterator) {
            if (ec) {
                return;
            }
            rsocket_.async_connect(*iterator, [this, self](std::error_code ec) {
                LOG_TRACE
                if (ec) {
                    return;
                }
...
void ServerSession::doWriteIV()
{
    ...
    rsocket_.async_read_some(asio::buffer(rbuf, sizeof(rbuf) - ivlen),
        [this, self](std::error_code ec, std::size_t length) {
            if (ec) {
                socket_.cancel(ec);
                return;
            ...
            asio::async_write(socket_, asio::buffer(rbuf, iv.length() + length),
                [this, self](std::error_code ec, std::size_t length) {
                    if (ec) {
                        rsocket_.cancel(ec);
                        return;
...
void ServerSession::doPipe1()
{
    LOG_TRACE
    auto self = shared_from_this();
    async_read_some([this, self](std::error_code ec, std::size_t length) {
        if (ec) {
            LOG_TRACE
            rsocket_.cancel(ec);
            return;
        ...
        LOG_TRACE
        asio::async_write(rsocket_, asio::buffer(buf, length),
            [this, self](std::error_code ec, std::size_t length) {
                if (ec) {
                    LOG_TRACE
                    socket_.cancel(ec);
...
void ServerSession::doPipe2()
{
    ...
    rsocket_.async_read_some(
        asio::buffer(rbuf, 16384), [this, self](std::error_code ec, std::size_t length) {
            if (ec) {
                LOG_TRACE
                socket_.cancel(ec);
                return;
            ...
            LOG_TRACE
            async_write(length, [this, self](std::error_code ec, std::size_t length) {
                if (ec) {
                    LOG_TRACE
                    rsocket_.cancel(ec);
                    return;
...
void ServerSession::async_read_with_timeout(
    std::size_t length, asio::high_resolution_timer::duration td, Handler handler)
{
    ...
    asio::async_read(socket_, asio::buffer(buf, length),
...
void ServerSession::async_read_with_timeout_1(
    std::size_t length, asio::high_resolution_timer::duration td, Handler handler)
{
    async_read_with_timeout(length, td, [handler, this](std::error_code ec, std::size_t length) {
        if (!ec) {
            dec_->decrypt(buf, length, buf, length);
        }
        handler(ec, length);
