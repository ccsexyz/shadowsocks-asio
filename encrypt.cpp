...
std::string evpBytesToKey(std::string password, int keyLen) {
    ...
    for(int i = 1; i < cnt; i++) {
        ...
        std::copy_n(psdMd5Sum2.cbegin(), std::min(static_cast<int>(psdMd5Sum2.length()), cnt*md5Len-start), vc.begin() + start);
    }
    return std::string(vc.cbegin(), vc.cbegin()+keyLen);
}

Cipher::Cipher(std::string method, std::string password) {
    enc_.SetKeyWithIV(reinterpret_cast<const byte *>(key_.c_str()), key_.length(),
...
std::string Cipher::decrypt(std::string src) {
    std::string secret;
    StringSource(src, true, new StreamTransformationFilter(enc_, new StringSink(secret)));
    return secret;
}

std::string Cipher::encrypt(std::string src) {
    std::string plain;
    StringSource(src, true, new StreamTransformationFilter(dec_, new StringSink(plain)));
    return plain;
}
