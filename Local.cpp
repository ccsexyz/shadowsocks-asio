...
Local::Local(asio::io_service &io_service, const config &config)
    ...
    , acceptor_(io_service,
          asio::ip::tcp::endpoint(
              asio::ip::address::from_string(config.local_addr), config.local_port))
...
void LocalSession::doSocks5HandShakePhase1()
{
    auto self = shared_from_this();
    asio::async_read(
        socket_, asio::buffer(buf, 2), [this, self](std::error_code ec, std::size_t length) {
            if (ec) {
                return;
            }
            auto ver = buf[0];
            ...
            auto handler = [this, self](std::error_code ec, std::size_t length) {
                ...
                buf[0] = 5;
                buf[1] = 0;
                asio::async_write(socket_, asio::buffer(buf, 2),
            ...
            if (nmethods > 0) {
                asio::async_read(socket_, asio::buffer(buf, nmethods), handler);
            } else {
...
void LocalSession::doSocks5HandShakePhase2()
{
    auto self = shared_from_this();
    asio::async_read(
        socket_, asio::buffer(buf, 6), [this, self](std::error_code ec, std::size_t length) {
            if (ec) {
                return;
            }
            auto ver = buf[0];
...
{
    auto self = shared_from_this();
    asio::async_read(socket_, asio::buffer(buf + 6, lenIPv4),
        [self, this](std::error_code ec, std::size_t length) {
            if (ec) {
                return;
            }
            auto address = asio::ip::address_v4(ntohl(*reinterpret_cast<long *>(buf + 4)));
            auto port = ntohs(*reinterpret_cast<uint16_t *>(buf + 4 + lenIPv4));
            auto header = std::string(buf + 3, 1 + lenIPv4 + lenPort);
            log_info("connect to %s:%d", address.to_string().c_str(), port);
            doEstablish(header);
        });
}
...
{
    auto self = shared_from_this();
    asio::async_read(socket_, asio::buffer(buf + 6, lenIPv6),
        [self, this](std::error_code ec, std::size_t length) {
            if (ec) {
                return;
            }
...
void LocalSession::doSocks5HandleDm()
{
    ...
    asio::async_read(socket_, asio::buffer(buf + 6, 1 + len),
        [self, len, this](std::error_code ec, std::size_t length) {
            ...
            log_info("connect to %s:%d", address.c_str(), port);
            doEstablish(header);
        });
}
...
void LocalSession::doEstablish(const std::string &header)
{
    ...
    ivlen += hdrsz;
    auto self = shared_from_this();
    asio::ip::tcp::resolver::query query(config_.remote_addr, std::to_string(config_.remote_port));
    resolver_.async_resolve(query,
        [this, self, ivlen, header](
            std::error_code ec, asio::ip::tcp::resolver::iterator iterator) {
            if (ec) {
                return;
            }
            rsocket_.async_connect(*iterator, [this, self, ivlen, header](std::error_code ec) {
                if (ec) {
                    return;
                }
                rbuf[0] = 0x5;
                ...
                asio::async_write(socket_, asio::buffer(rbuf, 3 + header.length()),
                    [this, self, ivlen](std::error_code ec, std::size_t length) {
                        if (ec) {
                            return;
                        }
                        asio::async_write(rsocket_, asio::buffer(buf, ivlen),
                            [this, self](std::error_code ec, std::size_t length) {
                                if (ec) {
                                    return;
                                }
...
void LocalSession::async_read_some(Handler handler)
{
    rsocket_.async_read_some(
        asio::buffer(rbuf, 4096), [this, handler](std::error_code ec, std::size_t length) {
            if (!ec) {
...
void LocalSession::async_read(size_t len, Handler handler) { async_read(rbuf, len, handler); }

void LocalSession::async_read(char *buffer, size_t len, Handler handler)
{
    asio::async_read(rsocket_, asio::buffer(buffer, len),
...
void LocalSession::async_write(size_t len, Handler handler) { async_write(buf, len, handler); }

void LocalSession::async_write(char *buffer, size_t len, Handler handler)
{
    enc_->encrypt(buffer, len, buffer, len);
    asio::async_write(rsocket_, asio::buffer(buffer, len),
...
void LocalSession::doPipe1()
{
    ...
    async_read_some([this, self](std::error_code ec, std::size_t length) {
        ...
        asio::async_write(socket_, asio::buffer(rbuf, length),
            [this, self](std::error_code ec, std::size_t length) {
                if (ec) {
                    rsocket_.cancel(ec);
                    return;
...
void LocalSession::doPipe2()
{
    ...
    socket_.async_read_some(
        asio::buffer(buf, 4096), [this, self](std::error_code ec, std::size_t length) {
            if (ec) {
                rsocket_.cancel(ec);
                return;
            }
...
void LocalSession::doReadIV()
{
    ...
    asio::async_read(rsocket_, asio::buffer(rbuf, enc_->getIvLen()),
        [this, self](std::error_code ec, std::size_t length) {
            if (ec) {
                socket_.cancel(ec);
                return;
