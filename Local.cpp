...
LocalSession::LocalSession(boost::asio::io_service &io_service,
    ...
    : service_(io_service), socket_(std::move(socket)), rsocket_(io_service) {}
...
void LocalSession::doSocks5HandleAtyp1() {
    auto self = shared_from_this();
    boost::asio::async_read(
        socket_, boost::asio::buffer(buf + 6, 4),
        [self, this](boost::system::error_code ec, std::size_t length) {
            if (ec) {
                return;
            }
            char *address = buf + 128;
            if (::inet_ntop(AF_INET, reinterpret_cast<void *>(buf + 4), address,
                            1024) == nullptr) {
                return;
            }
            std::string name = address;
            std::string port =
                std::to_string(ntohs(*reinterpret_cast<uint16_t *>(buf + 8)));
        });
}
    ...
    boost::asio::async_read(
        ...
        [self, len, this](boost::system::error_code ec, std::size_t length) {
            ...
            std::string port = std::to_string(
                ntohs(*reinterpret_cast<uint16_t *>(buf + 5 + len)));
        });
}
